...
const errorOnInvalid = args.includes("--error-on-invalid") || args.includes("--strict")
const checkUrls = args.includes("--check-urls") || args.includes("--validate-urls")
...
const validateUrl = async (url: string): Promise<{ valid: boolean; status?: number; error?: string }> => {
  // Check if it's a local file path (starts with /)
  if (url.startsWith("/")) {
    return validateLocalFile(url)
  }
...
const validateLocalFile = (filePath: string): { valid: boolean; status?: number; error?: string } => {
  try {
...
const processFile = async (file: string) => {
  ...
  if (data.logo) {
    urlValidations.push(
      validateUrl(data.logo).then((validation) => {
        if (!validation.valid) {
          urlErrorCount++
          warningCount++
          console.warn(`⚠️  Logo URL issue in ${relativePath}: ${validation.error}`)
          if (errorOnInvalid) {
  ...
  if (data.heroImage) {
    urlValidations.push(
      validateUrl(data.heroImage).then((validation) => {
        if (!validation.valid) {
          urlErrorCount++
          warningCount++
          console.warn(`⚠️  Hero image URL issue in ${relativePath}: ${validation.error}`)
          if (errorOnInvalid) {
  ...
  if (data.urls && typeof data.urls === "object") {
    for (const [urlType, url] of Object.entries(data.urls)) {
      if (url && typeof url === "string") {
        urlValidations.push(
          validateUrl(url).then((validation) => {
            if (!validation.valid) {
              urlErrorCount++
              warningCount++
              console.warn(`⚠️  URL issue in ${relativePath} (${urlType}): ${validation.error}`)
              if (errorOnInvalid) {
...
const projectMatter = processedFiles.filter(({ valid }) => valid).map(({ result }) => result.data)
...
if (noWrite) {
  ...
  console.log(`Would write ${projectMatter.length} projects to src/lib/projects.json`)
} else {
  ...
  console.log(`\n✅ Written ${projectMatter.length} projects to ${path.relative(process.cwd(), outputPath)}`)
}
